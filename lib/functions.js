...
`d = {
    ...
    data: {
        version: "${version}"
            ...
            print("red", `the ${name} directory already exists`);
        }
    });
};
const newDav = name => {
    return new Promise(done => {
        ...
        // exist component ? 
        if(fs.existsSync(dir)){
            print("red" ,`It already exist "${name}" component`);
            return done();
        } else {
            fs.mkdirSync(dir);
        }
...
const removeDav = name => {
    ...
    if (!fs.existsSync(url)) {
        print("red", "No exist component");
    }
    fs.removeSync(url);
    unLink(name, false);
    print("green",`removed component ${name}`);
}
